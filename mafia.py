...
def home():
    ...
    with lock:
        if game_started and 'player_name' in session:
...
def join():
    ...
    with lock:
        ...
        # Add player with session ID for identification
        players.append({"name": name, "session_id": session.get('session_id', request.remote_addr)})
...
def leave():
    ...
    with lock:
        ...
        # Verify this session owns this player name
        if session.get('player_name') != player_name:
            return redirect(url_for("home", error="Unauthorized."))
        ...
        # Also remove from assignments if they were assigned
        if player_name in assignments:
